  ...
  if(type == DT_UNDO_HISTORY)
  {
    ...
    while (l)
    {
      ...
      if (hitem->module == NULL)
      {
        ...
        if(dt_iop_load_module(module, base->so, base->dev)) return;
        ...
        if (!done)
        {
          ...
          struct _cb_data data = { module, hitem->multi_priority };
          dt_undo_iterate (darktable.undo, DT_UNDO_HISTORY, &data, &_undo_items_cb);
          done = TRUE;
        }
